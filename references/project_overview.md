...
| **Modeling** | Train baselines (e.g., Logistic Regression on TF-IDF, LSTM) on sentiment; log experiments (params, metrics), progress to advanced (e.g., BERT via Hugging Face for better sentiment nuance). Select best model via cross-validation. Prototype inference endpoint. | MLflow for tracking; `train_model.py`/`predict_model.py` in `src/models`. Dockerize training env. | **Reliability**: A/B testing in MLflow. **Scalability**: GPU support via AWS EC2. **Maintainability**: Serialized models in `models/` with metadata. **Adaptability**: Experiment with Hugging Face for zero-shot models. |
